...
Usage:
    manage.py (drive) [--js] [--log=INFO] [--camera=(single|stereo)] [--optimized]
...
Options:
    ...
    --optimized       Run path through TUM racing line optimizer before autopilot.
...
from parts.TUM_optimizer_part import TUMPathOptimizer

logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO)

def drive(cfg, use_joystick=False, camera_type='single', optimized=False):
    '''
    ...
    def load_path(filename=None):
        filename = filename or cfg.PATH_FILENAME
        if os.path.exists(filename) and path.load(filename):
           print("The path was loaded was loaded from ", filename)
