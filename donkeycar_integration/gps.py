...
try:
    ...
    from fusion_engine_client.messages.pose import PoseMessage
...
class GpsPosition:
    """
    Donkeycar part to read POSE or NMEA from serial port and convert to position
    """
    def __init__(self, serial: SerialPort, debug=False):
        self.debug = debug
        self.serial = serial
        self.decoder = FusionEngineDecoder() if FUSION_ENGINE_AVAILABLE else None
        self.serial_conn = serial.start()
        self.position = None
        self._start()

    def _start(self):
        while self.position is None:
            logger.info("Waiting for GPS POSE or NMEA...")
    ...
    def run_once(self):
        try:
            ...
            ts = time.time()
            if raw:
                raw = raw.decode(errors='ignore').strip()

                # Handle POSE binary messages
                if FUSION_ENGINE_AVAILABLE:

                    try:
                        data = bytearray(raw.encode(errors='ignore'))
                    except Exception as e:
                        logger.info("[FusionEngine] Byte conversion failed")
                        return None

                    try:
                        msgs = self.decoder.on_data(data)
                    except Exception as e:
                        logger.info("[FusionEngine] Decoding error")
                        return None
                    
                    # —————————————————————————————————————————————————————
                    # Modified Section: Point One Nav Fusion
                    # This part has been extended to read fused PoseMessages
                    # from the Point One Navigation Fusion Engine and convert
                    # them (latitude/longitude → UTM) into the (x, y) format
                    # expected by DonkeyCar.
                    # —————————————————————————————————————————————————————
                    ...
                    for msg in msgs:
                        if isinstance(msg, PoseMessage):
                            try:
                                utm_pos = utm.from_latlon(msg.latitude_deg, msg.longitude_deg)
                                logger.debug(f"[FusionEngine] PoseMessage: lat={msg.latitude_deg}, lon={msg.longitude_deg}")
                                return (ts, float(utm_pos[0]), float(utm_pos[1]))
                            except Exception as e:
                                logger.info("[FusionEngine] UTM conversion failed")
                                return None
                else:
                    logger.info("FusionEngine is not available at runtime but required for POSE decoding.")
                    return None

                # Fallback: Try parsing as NMEA
                position = parseGpsPosition(raw, self.debug)
                if position:
                    return (ts, position[0], position[1])

        except Exception as e:
            logger.info(f"[GPS] Failed to read or decode GPS data: {e}")
        return None
...
# [All other existing functions and classes from your original gps.py code remain untouched]

# Note: only the GpsPosition class and import headers have been modified to support POSE decoding inline.
